...
from deepcas9.model.swe import SWE_Pooling
...
class ESM2Finetune(LightningModule):
    def __init__(self,
                 ...
                 task: str = 'esm2-12layers-[ft-10-11]'):
        super(ESM2Finetune, self).__init__()
        self.esm_model = self._load_esm2(esm_file, fine_tune_layers, task)
        if "12layers" in task:
        ...
        if self.pool_strategy == 'swe':
            if "12layers" in task:
                self.pooling = SWE_Pooling(480, 50, 480)
            elif "33layers" in task:
                self.pooling = SWE_Pooling(1280, 50, 1280)
    ...
    def configure_optimizers(self):
        ...
        # optimizer = DeepSpeedCPUAdam(self.parameters(), lr=self.lr, weight_decay=self.weight_decay)
        return optimizer
    ...
    def _loss_fn(self, batch, mode):
        ...
        elif self.pool_strategy == 'swe':
            sequence_representations = self.pooling(sequence_representations)
    ...
    def _load_esm2(self, esm_file, fine_tune_layers, task):
        esm = ESM2(esm_file=esm_file, device=self.device)
        if "12layers" in task:
            if fine_tune_layers == "all":
                pass
            elif fine_tune_layers != "":
                esm.unfreeze_layers(layer_idxs=[int(i) for i in fine_tune_layers.split(',')])
            else:
    ...
    #         for i in range(4):
    #             self.zero_grad()
    #             pam_pred[:, i].backward(retain_graph=True)  # Retain graph for subsequent backward passes
    #             contribution_scores.append(
    #                 prot_sequence.grad.abs().mean(dim=0).cpu().numpy())  # Average gradient magnitude across batch
    #
    #     # Convert list of numpy arrays to a single numpy array for easier plotting
    #     contribution_scores = np.stack(contribution_scores, axis=0)
    #
    ...
    #     buf.seek(0)
    #     image = Image.open(buf)
    #     wandb.log({'test/contribution_scores': [wandb.Image(image)]})
    #
    #     return contribution_scores
