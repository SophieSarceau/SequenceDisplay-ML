...
class ESM2Finetune(LightningModule):
    ...
    def inference_multiple_prot(self, prot_sequences: list) -> Any:
        """
        Inference the PAM mutation prediction for multiple protein sequences
        """
        self.eval()
        with torch.no_grad():
            seq_list = [('id', prot_sequence) for prot_sequence in prot_sequences]
            sequence_representations = self.esm_model.esm_inference(seq_list, device=self.device).squeeze()
            sequence_representations = sequence_representations[:, self.last_embed_region:, :]
            if self.pool_strategy == 'mean':
                sequence_representations = torch.mean(sequence_representations, dim=1)
    ...
    def virtual_screening(self, save_dir):
        ...
        # loop vs_loader and calculate the prediction
        self.eval()
        limit_batch_save = 10000
        pam_pred_list = []
        mutation_list = []
        with tqdm(total=len(self.vs_loader), desc="Virtual Screening Progress", unit="batch") as pbar:
            for idx, batch in enumerate(self.vs_loader):
                prot_sequence, mutation = batch
                pam_pred = self.inference_multiple_prot(prot_sequence)
                pam_pred_list.extend(pam_pred)
                mutation_list.extend(mutation)
                if idx % limit_batch_save == 0:
                    with open(f'{save_dir}/vs_result_{idx}.pkl', 'wb') as f:
